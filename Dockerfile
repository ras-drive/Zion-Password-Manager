...
RUN ["/bin/bash", "-c", "cd Zion-Password-Manager; source $HOME/.cargo/env; make build --release"]

EXPOSE 8080

CMD ["/bin/bash", "-c", "cd Zion-Password-Manager; source $HOME/.cargo/env; make run --release"]
